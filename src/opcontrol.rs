...
pub fn opcontrol() {
    ...
    loop {
        ...
        // only change the motor voltages if the controller is connected (ly and ry are Some)
        if let (Some(ly), Some(ry)) = (ly, ry) {
            // move the motors based upon the joystick values
            motor::move_i8(LEFT_DRIVE_PORT, false, ly);
            motor::move_i8(RIGHT_DRIVE_PORT, true, ry);
        }
